...
m = 4 # 生成多項式の次数 m = deg(g(x)) ※これが検査記号数(次数4のため修正)
gx = 0b10011 # 偶数パリティ符号の生成多項式 G(x) = x^4 +　x + 1 の２進表現(10011)
k = 11 # 情報記号数((4)用に修正)
n = 15 # 符号長((4)用に修正)

for i in range(1<<(k)):
    print(format(i,'011b'),format(encoding(i),'015b')) #情報記号数と符号長に合う用に修正
