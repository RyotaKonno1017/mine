...
m = 3 # 生成多項式の次数 m = deg(g(x)) ※これが検査記号数 (次数3のため修正)
gx = 0b1011 # 偶数パリティ符号の生成多項式 G(x) = x^3 +　x + 1 の２進数表現(1011)
k = 4 # 情報記号数((3)用に修正)
n = 7 # 符号長((3)用に修正)

for i in range(1<<(k)):
    print(format(i,'04b'),format(encoding(i),'07b')) #情報記号数と符号長に合わせて修正
